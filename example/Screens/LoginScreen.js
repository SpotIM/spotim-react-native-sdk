import { ActivityIndicator, Platform, ScrollView, StyleSheet, Text, View, Button } from 'react-native';
import React, { Component } from 'react';
import { SpotIM, SpotIMAPI, SpotIMEventEmitter } from '@spot.im/react-native-spotim';
import {ANDROID_SSO_ENDPOINT_LOGIN, SSO_ENDPOINT_CODE_B_URL, SSO_ENDPOINT_LOGIN, USERNAME, PASSWORD, ACCESS_TOKEN_NETWORK} from '../Consts'
...
export default class LoginScreen extends Component {
  ...
  updateStatus() {
    getUserStatus().then(status=>{this.setState({userStatus: status})})
  }
    ...
    Http.onreadystatechange=(e) => {
      if(Http.readyState == 4) {
        ...
        .then((response) => {
            ...
            getCodeB(codeA, token, USERNAME).then(() => {
              this.setState({isLoading: false})
